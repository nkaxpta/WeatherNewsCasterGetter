function getCasterData() {
  ...
  const todayFormat = Utilities.formatDate(dateToday, "JST", "M月d日");

  // シート名指定の準備
  const sheetName = Utilities.formatDate(dateToday, "JST", "yyyy/MM");

  // 翌日の日付・月を取得 → 整形
  const dateTomorrow = new Date(dateToday.getFullYear(), dateToday.getMonth(), dateToday.getDate() + 1);
  const tomorrowFormat = Utilities.formatDate(dateTomorrow, "JST", "M月d日");
  const tomorrowMonth = Utilities.formatDate(dateTomorrow, "JST", "M");
  ...
  // 翌月のシート名指定の準備
  const nextSheetName = Utilities.formatDate(nextMonth, "JST", "yyyy/MM");

  //----------------------------------------------------------------
  // スプレッドシート指定 → アクティブシートと翌月のシートの設定
  const spreadSheet = SpreadsheetApp.getActiveSpreadsheet();
  const activeSheet = spreadSheet.getSheetByName(sheetName);
  const nextSheet = spreadSheet.getSheetByName(nextSheetName);
  ...
  const res = UrlFetchApp.fetch(TIMETABLE_URL);
  ...
  // console.log("today：%o", today)
  // console.log("tomorrow：%o", tomorrow)
  // console.log("tomorrow.month：%s", tomorrow[0].month)
